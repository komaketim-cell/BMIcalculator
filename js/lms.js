...
const calcTargets = (tdee) => {
  return {
    ...
    leanGain: Math.round(tdee * 1.1),
    muscleGain: Math.round(tdee * 1.15),
    fatLoss: Math.round(tdee * 0.85),
  };
};
...
const buildRecommendations = ({
  ...
  if (status.includes("لاغری")) {
    ...
    recs.push("کاهش کالری کنترل‌شده به‌همراه تمرین هوازی+قدرتی توصیه می‌شود.");
  ...
  if (goal === "muscle") {
    ...
    recs.push("افزایش وزن با غذاهای مغذی و نه صرفاً کالری خالی انجام شود.");
  }
...
const computeAll = ({
  ...
  goal,           // "maintenance" | "gain" | "muscle" | "fatloss"
}) => {
